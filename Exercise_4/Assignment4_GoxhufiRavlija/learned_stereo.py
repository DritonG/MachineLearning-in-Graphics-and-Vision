...
class StereoMatchingNetwork(torch.nn.Module):
    def __init__(self):
        ...
        self.layer = torch.nn.Sequential(
            torch.nn.Conv2d(1, filter_number, 3),
            torch.nn.ReLU(),
            torch.nn.Conv2d(filter_number, filter_number, 3),
            torch.nn.ReLU(),
            torch.nn.Conv2d(filter_number, filter_number, 3),
            torch.nn.ReLU(),
            torch.nn.Conv2d(filter_number, filter_number, 3),
    ...
    def forward(self, X):
        ...
        # TODO: ENTER CODE HERE (EXERCISE 4.2 a))
        # -------------------------------------
        out = self.layer(X.permute(0, 3, 1, 2))
...
def calculate_similarity_score(infer_similarity_metric, Xl, Xr):
    ...
    # -------------------------------------
    feature_left = infer_similarity_metric.cuda()(Xl)
    feature_right = infer_similarity_metric.cuda()(Xr)
    score = torch.mm(feature_left[:,0,0,:], feature_right[:,0,0,:].T)
